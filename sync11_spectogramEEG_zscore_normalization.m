...
%%
% Z-score normalize the average spectrogram
mean_spectrogram = mean(average_spectrogram(:));
std_spectrogram = std(average_spectrogram(:));
z_score_normalized_spectrogram = (average_spectrogram - mean_spectrogram) / std_spectrogram;
%%
% Plot the z-score normalized average spectrogram
figure;
subplot(2,1,1)
...
colorbar
 colorbar_label= '(dB)'; % Add colorbar with unit label
 ylabel(colorbar, colorbar_label);
% colorbar('Ticks', [-15, 0, 10], ...
    % 'TickLabels', {'-15 dB', '0 dB', '10 dB'}); % Add colorbar with unit label
...
% subplot(2,1,1)
% imagesc(epochs{1}.T, epochs{1}.F(desired_freq_start_index:desired_freq_end_index), ...
%     abs(average_spectrogram(desired_freq_start_index:desired_freq_end_index, :))); % Remove the log scaling
% colormap(jet);
% axis xy;
% xlabel('Time (s)');
% ylabel('Frequency (Hz)');
% title('Average Spectrogram');
% colorbar
% colorbar_label= 'μV^2/Hz'; % Add colorbar with unit label
% ylabel(colorbar, colorbar_label);
% %  ...
% % 
...
subplot(2,1,2)
imagesc(epochs{1}.T, epochs{1}.F(desired_freq_start_index:desired_freq_end_index), ...
    10*log10(abs(z_score_normalized_spectrogram(desired_freq_start_index:desired_freq_end_index, :))));
colormap(jet);
axis xy;
xlabel('Time (s)');
ylabel('Frequency (Hz)');
title('Z-Score Normalized Average Spectrogram');
colorbar;

% Add colorbar label
colorbar_label = '(Z-Score)';
colorbar('Ticks', [-3, -2, -1, 0, 1, 2, 3], 'TickLabels', {'-3σ', '-2σ', '-1σ', '0', '1σ', '2σ', '3σ'});
ylabel(colorbar, colorbar_label);
