...
disp('Enter ROI:');
disp('Enter ROI:');
disp('1 for sample Data between -15s to -10s');
disp('2 for sample Data between -10s to -5s');
disp('3 for sample Data between -5s to 0s');
disp('4 for sample Data between 0s to 5s');
disp('5 for sample Data between 5s to 10s');
disp('6 for sample Data between 10s to 15s');
ROI = input('Enter ROI (1-6): ');
...
elseif ROI == 6
    ...
    disp('Invalid ROI input.');
    return;
end

% Display the mapped values
disp(['ROI1:', num2str(ROI1)]);
disp(['ROI2:', num2str(ROI2)]);
%%
if eventIndex==1
...
% Iterate over each event
    ...
    for combinationIndex = 1:length(normalized_combinationMatrices)
        ...
        axis square;
if eventIndex==1
    sgtitle(['Event: ', events(1)])
elseif eventIndex==2
    sgtitle(['Event: ', events(2)])
elseif eventIndex==3
    sgtitle(['Event: ', events(3) ])
end
